...
for location in $(head -c4096 $1 |xxd -p -c 4); do
	...
	#log_debug '[DEBUG] tail -c+$(('"$offset"'*4096+1)) '"$1"'|head -c8|xxd -p -c 8'
	...
	#log_debug '[DEBUG] chunk_length=$(((16777216 * 0x'"${chunkheader:0:2})"' + (65536 * 0x'"${chunkheader:2:2}"') + (256 * 0x'"${chunkheader:4:2}"') + 0x'"${chunkheader:6:2}"'))'
	...
	if [[ $chunk_length -gt 0 ]]; then	#skip zero-length chunks
		...
		echo "[INFO] Extracting chunk $chunk_number (modify time: $(date -d @$timestamp)) to out/$chunk_number.nbt"
